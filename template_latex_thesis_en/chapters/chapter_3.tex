\section{Gauss reduction - single core case}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Algorithms in \lt{{\sc{LaTex}}}}    

\begin{algorithm}[H]
 \footnotesize {
\begin{algor} : {Multiplication of \lt{Karatsuba}}\\
{\bf input.} $a,b$ integers\\
{\bf output.} $a\cdot b$
\end{algor}
{\lt{
 \SetAlgoLined
...
{\footnotesize{
\noindent
\begin{algor} : {Enumeration algorithm}\label{al-enum}\\
{\bf Είσοδος.}  An ordered basis $\mathcal{B}=\{{\bf b}_1,\dots, {\bf{b}}_n\}\subset\mathbb{Z}^{m}$ 
of the lattice $\mathcal{L}(\mathcal{B})$ and a positive real number $R.$ \\
{\bf Έξοδος.} All the vectors${\bf x}\in \L$ με $||{
\bf x}||\leq R.$
\end{algor}
\lt{\ \\
